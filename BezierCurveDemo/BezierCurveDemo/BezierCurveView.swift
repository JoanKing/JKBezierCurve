...
class BezierCurveView: UIView {
    ...
    var points: [CGPoint] = []
    /// 渐变曲线
    let lineLayer = LineGradientLayer()
    /// 曲线上所有的点
    var allPointList: [CGPoint] = []
    /// 上传点的数组
    var paramPoints: [CGPoint] = []
    /// 取值的点的圆圈视图
    var paramCircleViews: [UIView] = []
    /// 手势点击的有效距离
        ...
        case .changed:
            if let panView = panGesture.view {
                ...
                if (panView.frame.minY + translation.y) >= 0 && (panView.frame.maxY + translation.y) <= frame.size.height && (panView.frame.minX + translation.x) >= 0 && (panView.frame.maxX + translation.x) <= frame.size.width {
                    ...
                    // let value = 1.0 - (panView.frame.midY / frame.size.height)
                    // let x_value = (panView.frame.midX / frame.size.width)
                    // debugPrint("x_value: \(x_value)")
                    let point = CGPoint(x: panView.frame.midX, y: panView.frame.midY)
                                    
                    guard (panView.tag != 0 && panView.tag != points.count - 1), removePointBeyondFrontAndRearPoints else {
    ...
    /// 添加圆圈视图
    func setCircleView(index: Int, point: CGPoint) {
        let view = CircleView()
        view.layer.cornerRadius = 7.5
        view.clipsToBounds = false
        view.backgroundColor = .randomColor
...
extension BezierCurveView {
    ...
    func drawPath(path: CGMutablePath) {
        // 添加路径到图形上下文
        let colors: [CGColor] = [UIColor.hexStringColor(hexString: "#367FE5").cgColor, UIColor.hexStringColor(hexString: "#DF001F").cgColor]
        
        lineLayer.colors = colors
        
        lineLayer.startPoint = CGPoint(x: 0, y: 0.5)
        lineLayer.endPoint = CGPoint(x: 1, y: 0.5)
        lineLayer.locations = [0, 0.2]
...
extension BezierCurveView {
    ...
    ///   - path: CGMutablePath
    /// - Returns: description
    func getPointXY(xy: CGFloat, path: CGPath, isX: Bool = true) -> CGPoint {
...
extension BezierCurveView {
    ...
    /// 手势的点是否在线上，有一个合理的距离
    func isPointLine(point: CGPoint) -> (isEffectivePoint: Bool, insertIndex: Int) {
        guard let currentPath else {
            return (false, 0)
        }
        // 1、点击点首先要在 左右两个点的矩形内，如果不在不生点
